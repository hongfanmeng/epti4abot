...
enum DIRECTION_enum { DIRECTION_ADV, DIRECTION_BACK };
...
class CarController {
 public:
  ...
  CarController(const std::string& deviceName, const unsigned baud)
      : serial_(Serial(deviceName, baud)) {
    if (!serial_.isOpened()) {
  ...
  inline void syncCarLeft() const {
    ...
    while (!serial_.sendline(command)) usleep(10000);
  }
  ...
  struct Motor left() const { return mLeft_; }
  struct Motor right() const { return mRight_; }
